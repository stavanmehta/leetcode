...
class Solution {
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        Map<String, Integer> subDomainCount = new HashMap<String, Integer>();
        
        for (String domain:cpdomains) {
            String[] array = domain.split("\\s");
            String count =  array[0];
            String[] subDomains = array[1].split("\\.");
            subDomainCount.put(array[1], subDomainCount.getOrDefault(array[1],0)+Integer.parseInt(count));
            if (subDomains.length == 2)
                subDomainCount.put(subDomains[subDomains.length-1], subDomainCount.getOrDefault(subDomains[subDomains.length-1],0)+Integer.parseInt(count));
            else {
                subDomainCount.put(subDomains[subDomains.length-2]+"."+subDomains[subDomains.length-1], subDomainCount.getOrDefault(subDomains[subDomains.length-2]+"."+subDomains[subDomains.length-1],0)+Integer.parseInt(count));
                subDomainCount.put(subDomains[subDomains.length-1], subDomainCount.getOrDefault(subDomains[subDomains.length-1],0)+Integer.parseInt(count));
            }
        }
        ...
        for (String key:subDomainCount.keySet()) {
            ...
            value.append(subDomainCount.get(key));
            value.append(" ");
            value.append(key);
