...
class Solution {
    public String nextClosestTime(String time) {
        int timeInMinutes = 60 * Integer.parseInt(time.substring(0,2));
        timeInMinutes += Integer.parseInt(time.substring(3));
        
        Set<Integer> allowedDigits = new HashSet<Integer>();
        ...
        while (true) {
            ...
            int[] digits = new int[]{timeInMinutes/60 / 10, timeInMinutes/60 % 10, timeInMinutes % 60 / 10, timeInMinutes % 60 % 10};
            search: {
                for (int d: digits) if (!allowedDigits.contains(d)) break search;
